...
class Complaint(db.Model):
    ...
    # User estimated completion time (in minutes, optional, max 60 minutes)
    estimated_completion_time = db.Column(db.Integer, nullable=True)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'estimated_completion_time_formatted': self._format_completion_time() if self.estimated_completion_time else None,
            'feedback': self.feedback.to_dict() if self.feedback else None,
            'assignment': self.assignment.to_dict() if self.assignment else None
        }
    
    def _format_completion_time(self):
        """Format estimated completion time in human-readable format"""
        if not self.estimated_completion_time:
            return None
        
        minutes = self.estimated_completion_time
        if minutes < 60:
