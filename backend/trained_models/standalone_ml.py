...
def classify_query(current_user_id, current_user_role):
    ...
    try:
        ...
        if len(query_text) < 5:
            return jsonify({'success': False, 'message': 'Query text must be at least 5 characters long'}), 400

        # Use sentence-transformer+SVM if available, otherwise rule-based fallback
        title       = data.get('title', '').strip()
        description = data.get('description', '').strip()

        try:
            from utils.bert_classifier import classify, is_bert_loaded
            if title or description:
                category, priority = classify(title, description)
            else:
                category, priority = classify(query_text)
...
def ml_status(current_user_id, current_user_role):
    ...
    try:
        from utils.bert_classifier import is_bert_loaded, get_labels
        bert_loaded = is_bert_loaded()
        labels = get_labels()
        categories = labels['categories'] if bert_loaded else list(ml_service.category_patterns.keys())
        priorities  = labels['priorities']  if bert_loaded else list(ml_service.priority_patterns.keys())
        method = 'BERT Transformer' if bert_loaded else 'CSV-based Pattern Matching'
    except Exception:
        bert_loaded = False
        categories = list(ml_service.category_patterns.keys())
        priorities  = list(ml_service.priority_patterns.keys())
        method = 'CSV-based Pattern Matching'
